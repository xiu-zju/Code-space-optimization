...
**错误信息**: `Error: gcc not found` 或 `Error: clang not found`
...
**错误信息**: `Permission denied: ./scripts/run_tests.sh`
...
**说明**: 这是正常行为。脚本会记录错误并继续处理其他测试。检查日志文件了解详细错误信息。
...
**解决方案**: 检查测试程序是否适合PGO优化，某些程序可能需要修改以支持自动化profile收集。

## 添加新测试程序

1. 将C源文件添加到 `src/` 目录
2. 确保程序有main函数且可以独立编译
3. 运行测试脚本，新程序会自动被检测和编译
...
这些程序涵盖了不同的计算模式和代码结构，有助于全面评估编译器优化效果。
